...
export function crossTierScheduleFingerprint() {
  return String(
    hashSeed(
      stableStringify({
        algorithm: "diagnostic-cross-tier-v15-v3-no-clock-zero-time-deterministic-ids-slot-stable-drafts-relocation-aware-worlds",
...
export function crossTierPairedSeed(
  ...
  return hashSeed(
    `${options.seed}:cross-tier-v15:${BALANCE_ENGINE_RULES_FINGERPRINT}:${group.cycle}:${group.key}:${group.scheduleOrdinal}:${group.roundOrder}`,
  );
}
