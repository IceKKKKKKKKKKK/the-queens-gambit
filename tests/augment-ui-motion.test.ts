...
test("lobby scenes return through a cancellable reducer while preserving focus provenance", () => {
  ...
  assert.match(lobbySource, /<h1>城堡大厅<\/h1>/);
  assert.match(lobbySource, /styles\.wordmarkSeal[\s\S]*?<i \/>/);
  assert.match(lobbySource, /styles\.accountCrest[\s\S]*?<i \/>/);
  assert.doesNotMatch(lobbySource, /styles\.(?:wordmarkSeal|accountCrest|tuckFace)[^\n]*>令</);
  assert.doesNotMatch(lobbySource, /styles\.loadingMark[^\n]*>令<\/span>/);
  assert.match(lobbyCss, /--ground:\s*#aaa99f/);
  assert.match(lobbyCss, /--wine:\s*#59242c/);
  assert.match(lobbyCss, /--brass:\s*#786740/);
  assert.match(lobbyCss, /--pewter:\s*#66665f/);
  assert.match(lobbyCss, /\.wordmarkSeal\s*\{[\s\S]*?clip-path:\s*polygon\(50% 0, 92% 13%, 86% 70%, 50% 100%, 14% 70%, 8% 13%\)/);
  assert.match(lobbyCss, /\.compass\s*\{[\s\S]*?width:\s*90%[\s\S]*?border:\s*3px double var\(--brass\)/);
