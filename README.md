...
在线二人暗军棋，用户入口统一为“经典 / 狂野 / 个人”。狂野模式在经典规则上增加两轮军令三选一；平台同时支持账号、邀请、匹配、棋钟、观战和逐手复盘。

本文中的 `augment`、`hex_ranked` 与旧“强化”称呼仅是内部技术标识和历史术语；用户可见玩法名称统一为“狂野”，牌面统一称为“军令”。

## 在线试玩

- 正式站点：https://yizhen-junqi.cuiyangzhi78.chatgpt.site/
- 支持经典好友房、狂野好友房、狂野排位、观战、好友、战绩与逐手复盘。
- 正式站点由 ChatGPT Sites 提供邮箱登录；本地开发使用下文的测试身份代理，不需要真实账号或私有凭据。
...
```powershell
git clone https://github.com/IceKKKKKKKKKKK/the-queens-gambit.git
cd the-queens-gambit
npm ci
npm run dev -- --hostname 127.0.0.1 --port 4317
```

开发服务器会使用项目内的本地 Cloudflare D1 状态；首次访问 API 时会自动创建所需表。另开一个终端启动仅供本机开发的身份代理：

```powershell
node scripts/qa/auth-proxy.mjs
```
...
| 地址 | 身份 |
|---|---|
| http://127.0.0.1:4318 | 黑方测试玩家 |
| http://127.0.0.1:4319 | 白方测试玩家 |
| http://127.0.0.1:4320 | 观战测试玩家 |

身份代理只应绑定并用于本机开发，不能作为生产认证方案。关闭开发服务器后，本地 D1 数据保留在被 Git 忽略的 `.wrangler/` 中；删除该目录即可重置本地数据。
...
- `app/`：页面、房间 API 与 UI；
- `lib/`：服务端权威棋局、军令、身份与排位规则；
- `db/` 与 `drizzle/`：D1 数据访问、schema 与迁移；
- `tests/`：规则、UI、API 与真实房间集成测试；
- `scripts/`：平衡模拟、持续测试和本地 QA 工具；
- `docs/`：狂野规则、验收与模拟说明。

项目采用 [MIT License](LICENSE)，可以使用、修改和再分发；提交改动前请至少运行相关测试、类型检查和 ESLint。
...
| 对局引擎指纹 | `augment-duel-dark-v3:threefold-3:strategic-sha256-v4:complete-augment-attribution-v1` |
| 当前结论 | 70 张已进入服务端规则；本次沉浸式纸牌 UI 候选以完整回归、真实 API、私有 Sites 预览和三档 Browser 验收为发布门槛，不安排新的四小时循环测试 |

`lib/augments.ts` 中的 `AUGMENT_CATALOG` 是运行时权威来源。机制参数指纹只覆盖稳定 ID、花色、次数和结构化效果，不覆盖名称或说明文案；规则改动后应同时更新本 README 和测试记录。

v18 暗棋采样沿用玩家可见投影、公开闪电战状态和公开晋升标记边界，在标准棋子库存容量内抽取基础军阶，再独立计算当前有效军阶；对手已锁但尚未共同公开的选牌只从合成选项恢复到内部 loadout，公开前 trigger 保持 0 且不标记 used。采样不会读取权威 `baseTypes`、私有选牌或回放私密信息，每个 v3 抽样世界都必须通过规则校验并重新投影为同一玩家可见状态。三次重复摘要忽略不会改变剩余次数或未来权利的持续牌触发计数，仍保留主动、自动、布阵牌次数以及棋子、公开信息、大本营、地雷、伤亡、闪电战和追加行动等规则状态；只有敌方棋子实际进入守方大本营才会解锁“濒死悟道”。事件与回放会按主动牌、即时结算牌、后续自动牌的顺序完整记录复合触发。旧 v17 算法、裸 `strategic-sha256-v4` 归因合同与更早引擎 checkpoint 均拒绝恢复。
...
狂野排位的标准棋钟是每方 10 分钟。合法落子扣除思考时间后，若该方剩余时间不高于 5 分钟，则完整返还 5 秒，因此恰好 5:00 会变为 5:05；布阵和第二轮选牌期间暂停对局棋钟，主动放弃追加行动只扣思考时间、不返还这 5 秒。

`junqi-augments-v2` 与 `junqi-augments-v3` 狂野对局在第二轮公开后启用三次重复局面和棋。相同已结算战略局面第二次出现时，双方看到公开的 `2/3` 提示；第三次出现立即和棋，排位按双方各 `0.5` 分结算。经典模式和持久化的 v1 狂野对局保持原规则，不采用这项裁决。
...
以下记录截至 2026-08-14 ET。`outputs/` 被 Git 忽略；它用于保存本机运行证据，不随源码提交。规则、参数、目录或算法改变后，旧报告与检查点只能作为历史记录，不能混入当前结论。

### 已有证据

- 最终完整回归 283/283 通过：HTML 9 项、集成服务器契约 4 项、TypeScript 267 项、真实集成 3 项；
...
在上述自动化和受支持的浏览器视口证据范围内没有已知 P0/P1。它仍不替代真人联机、跨浏览器、辅助技术或生产环境验证。

花色仍以 `黑桃 > 红桃 > 梅花 > 方块` 作为产品设计意图，但机器人胜率、档位点估计或排序不属于发布 gate，也不会触发自动改卡。首批玩家反馈和真实对局数据到齐后，再单独评审强度调整。

本次界面与动作协议发布不安排新的四小时循环测试。冻结候选必须通过完整回归、非增量类型检查、全项目 ESLint、production build、真实 API 集成、私有 Sites 候选，以及 1440×900、1024×768、390×844 的 Browser 功能/键盘/触摸/减弱动效/无横向溢出验收；同一已验证版本才可公开。四 worker soak 仍保留为可选的引擎与基础设施诊断工具，不是本次 UI 发布的硬门槛。
