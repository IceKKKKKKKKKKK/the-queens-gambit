...
- 引擎：`augment-duel-dark-v3:threefold-3:strategic-sha256-v4:complete-augment-attribution-v1`；三次重复摘要忽略持续牌纯触发计数，但保留有次数权利的计数与现有规则状态；复合触发的事件/回放完整保留主动、即时结算与后续自动牌；己方棋进入己方大本营不会解锁对手“濒死悟道”，裸 `strategic-sha256-v4` 归因合同及更早 checkpoint 均拒绝恢复。
