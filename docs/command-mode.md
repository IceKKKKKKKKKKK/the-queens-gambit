...
用户可见玩法名：狂野（内部 `augment` / `hex_ranked` 标识保持兼容，不迁移历史数据）
...
- 这项裁决属于 `junqi-augments-v2` 和 `junqi-augments-v3` 狂野对局。经典模式和已持久化的 v1 狂野对局继续按原规则运行，即使反复往返也不会被新版自动判和。
...
- 三张牌使用单选组语义；方向键、Home 和 End 只移动焦点，Enter、Space 或点击立即原子选择并锁定。
...
- 现行 UI 使用一次 `augment_pick` 完成选择与锁定，不显示独立锁定按钮；旧 `augment_select`、`augment_lock` 仅保留兼容，结果通过 `aria-live` 宣布。
...
- 桌面端双方军令轨统一显示在棋盘左侧，窄屏改为棋盘上方的横向卷轴；每方最多两张。
- 双方公开后，所有玩家和合法观众都能看到双方已选择的强化名称与是否已用。
- 公开牌悬停或聚焦显示非模态预览，点击打开检查对话框；只有本人未使用且处于合法时机的主动牌在对话框内显示“发动军令”，其余只读并显示原因。
...
- 在线玩家可进入“狂野排位”队列；服务器按分数区间、等待时长和连接地区逐步扩展搜索。
...
| 领域 | 必要字段 |
|---|---|
| 兼容 | 状态结构版本、规则集 ID、经典/狂野模式 |
| 军令目录 | 目录版本、70 张稳定定义及 v1 20 张、v2 50 张兼容池 |
...
### 9.5 截至 2026-08-12 ET 的冻结证据

- 最终完整回归 283/283 通过：HTML 9 项、集成服务器契约 4 项、TypeScript 267 项、真实集成 3 项；production build、全项目 ESLint、`tsc --noEmit --incremental false` 与 `git diff --check` 通过。
...
- 在当前自动化与受支持视口证据范围内没有已知 P0/P1。本次沉浸式纸牌 UI/动作协议发布以完整回归、真实 API、私有 Sites 候选和 1440×900、1024×768、390×844 Browser 验收为冻结提交后的 release gate；不安排新的四小时循环测试。四 worker soak 继续作为可选诊断，本机证据保留在 `outputs/`。
