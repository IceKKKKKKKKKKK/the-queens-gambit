...
- 第二轮使用与第一轮不同的花色，并在抽取时逐张排除 `activation=setup` 的布阵专属牌；方块中的中局牌仍可进入第二轮。已持久化的 v1 房间继续沿用“第二轮排除整个方块花色”的旧规则，不能静默改写旧局。
...
1. v2 目录共 50 张强化：黑桃 13、红桃 13、梅花 12、方块 12；v1 的旧 20 张 ID 与语义保持不变。
...
- 这项裁决只属于新建的 `junqi-augments-v2` 对局。经典模式和已持久化的 v1 强化对局继续按原规则运行，即使反复往返也不会被新版自动判和。
...
`lib/augments.ts` 中带目录版本的定义是运行时权威来源。v2 目录固定为黑桃 13、红桃 13、梅花 12、方块 12；每张牌包含稳定 ID、名称、短名称、花色、说明、触发时机、使用次数和结构化效果。已发布 ID 的语义不得原地修改；平衡修改需升级目录版本并保证旧复盘仍可解释。
...
| 花色 | 等级 | 数量 | 抽取约束 |
|---|---|---:|---|
| 黑桃 ♠ | 最高 | 13 | 13；第二轮不重复首轮花色 |
| 红桃 ♥ | 较高 | 13 | 13；第二轮不重复首轮花色 |
| 梅花 ♣ | 中等 | 12 | 12；第二轮不重复首轮花色 |
| 方块 ♦ | 基础 | 12 | 8；逐张排除 4 张布阵专属牌 |
...
- v2 三次重复局面按双方各 `0.5` 的和棋分结算；评分变化仍由双方赛前评分决定，不能把“和棋”错误写成固定零分变化。
...
- 动作前后双方棋钟和强化可用状态。
- 第二轮暂停、锁定、公开与恢复时点。
- v2 重复局面的公开计数、`draw_repetition` 事件与三次重复终局原因；经典和 v1 复盘不得被新版补判和棋。
- 终局结果、结束原因和排位结算引用。
...
- 50 张强化逐张覆盖合法、非法、边界、并发、超时、次数消耗和规则组合；旧 20 张另做持久化快照兼容回归。
...
### 9.5 截至 2026-08-10 ET 的冻结证据

- 最终邀请修复后的完整回归 200/200 通过：HTML 9 项、TypeScript 188 项、真实集成 3 项；生产构建、全项目 ESLint、`tsc --noEmit --incremental false` 与差异检查通过。
- 未登录玩家邀请已覆盖同标签页 `sessionStorage` 恢复、认证 URL/服务端 HTML 无 secret，以及 `watch=1` 不读取、不替换、不消费玩家邀请。
- 当前平衡算法为 `hidden-info-balance-v11-threefold-public-occurrence`，产品引擎指纹为 `augment-duel-dark-v2:threefold-3:strategic-sha256-v1`。
- v11 预检完成 142 个镜像组、568 条腿；强搜索完成 2 个 cycle、284 个镜像组、1,136 条腿。强搜索自然结束为夺旗 1,031、无棋可走 97、三次重复和棋 8，封顶、异常、卡死和第二轮时序违规均为 0；两份报告均为 `acceptancePass=true`。
- 该绿灯只证明跨档实验的方法、时序、覆盖和完整性门槛。六组比较和四档卡等权 95% 区间均跨过 50%，严格 `黑桃 > 红桃 > 梅花 > 方块` 的经验排序没有得到证明。
- 576 条腿的定向强搜索 A/B 没有让“扇区侦察”或“工兵誓约”的候选调整通过预注册门槛，两张牌均不调数值。
- 在当前自动化与受支持的 Chrome 视口证据范围内没有已知 P0/P1；正式连续不少于 4 小时且至少 288 组的同档 soak、提交、公开部署和生产回归尚未执行。

跨档强搜索使用每局最多 300 手；正式同档 soak 仍固定每局最多 180 手。跨档报告不能替代正式四 worker 持续测试，也不能替代真人暗棋盲测。

## 10. 上线顺序与熔断

1. 冻结经典模式基线，建立行为回归快照。
2. 完成账号、资料和权限边界，但先不开放排位。
3. 内部房启用 50 张 v2 目录、两轮选择、v1 兼容恢复和安全观战。
...
- 经典模式默认、现有规则与显示无回归。
- 50 张强化均属于正确花色且有完整规则、服务器实现和逐张测试；v1 旧房间仍按旧 20 张目录恢复与回放。
...
- 首次公开测试前，目标与完整代码检查、深度平衡、模拟、压力、安全、Chrome 390×844 / 768×1024 / 1440×1000 视觉回归均达到各自门槛；真实三人、Safari/Firefox、屏幕阅读器、360px 和 200% 缩放保留为公开测试后的扩展验证，不能伪写为已完成。
