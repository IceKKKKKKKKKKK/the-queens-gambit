...
恢复会保留历史样本，但打开新的连续段。旧段时长不能补入新段的四小时门槛。当前 soak schema 为 2；manifest 还绑定算法 `product-stability-v17-v3-no-clock-zero-time-deterministic-ids-slot-stable-drafts-relocation-aware-public-rank-capacity-worlds-private-draft-fidelity`、引擎 `augment-duel-dark-v3:threefold-3:strategic-sha256-v3`、实际 balance 配置、63 组赛程和工作区指纹。v17 暗棋世界只依据玩家可见投影与公开闪电战/晋升元数据，在标准库存容量内抽取基础军阶并独立生成当前军阶；对手已锁但未共同公开的卡只进入合成内部 loadout，公开前 trigger 为 0 且 unused。每个 v3 世界都通过规则校验和重投影一致性检查，不读取权威 `baseTypes`、私有选牌或回放私密信息。任何旧 schema、旧算法（包括 v13、v14、v15、v16）、旧引擎、旧工作负载或旧源码 checkpoint 都必须拒绝。
...
截至 2026-08-11 ET，最终代码回归为 266/266：HTML 9 项、集成服务器契约 4 项、TypeScript 250 项、真实集成 3 项；production build、全项目 ESLint、非增量 TypeScript 检查与差异检查均通过。三套真实集成各连续执行 4 轮，共 12/12 通过；每轮端口可重新绑定，结束后孤儿进程为 0。API 集成另在同一隔离工作区连续执行 64/64 次且零自动重试，全部返回 JSON、64 个动态端口均可重绑、D1 文件与 Vinext/Workerd 残留均为 0；独立双启动哨兵验证显式持久状态可在停服后读回并安全清理。
