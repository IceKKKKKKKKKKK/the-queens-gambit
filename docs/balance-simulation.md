...
当前平衡 checkpoint/report schema 为 5，算法版本为 `product-stability-v15-v3-no-clock-zero-time-deterministic-ids-slot-stable-drafts-relocation-aware-worlds`，引擎指纹为 `augment-duel-dark-v3:threefold-3:strategic-sha256-v2`。配置指纹绑定实际零时间配置、eligible IDs、排除 IDs、搜索配置和赛程。任何旧 schema、旧算法（包括 v13、v14）、旧引擎、旧牌池、旧搜索或旧赛程 checkpoint 都必须拒绝，不能合并历史样本。
...
截至 2026-08-11 ET，最终代码回归为 251/251：HTML 9 项、集成服务器契约 4 项、TypeScript 235 项、真实集成 3 项；production build、全项目 ESLint、非增量 TypeScript 检查与差异检查均通过。三套真实集成各连续执行 4 轮，共 12/12 通过；端口每轮可重新绑定，结束后孤儿进程为 0。API 集成另在同一隔离工作区连续执行 64/64 次且零自动重试，全部返回 JSON、64 个动态端口均可重绑、D1 文件与 Vinext/Workerd 残留均为 0；独立双启动哨兵验证显式持久状态可在停服后读回并安全清理。
